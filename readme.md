# Circuitos Eletrônicos I — Análise e Simulação de Circuitos com Diodos (T3)

Este repositório contém a resolução e a validação computacional dos exercícios propostos na lista de estudos para o **Teste 3 (T3)** da disciplina de **Circuitos Eletrônicos I**.

O objetivo principal do projeto é confrontar os cálculos de aproximação linear feitos à mão (como o modelo de queda de tensão constante $V_D$) com o comportamento não linear real de circuitos semicondutores obtido através de simulações numéricas do motor **SPICE**.
